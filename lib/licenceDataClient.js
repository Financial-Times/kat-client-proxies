...
function getFilteredUserList (licenceId, apiAuthToken, options) {
	...
	return fetch(url, _getOptions(apiAuthToken))
		...
		.then(res => {
			log.debug({operation, licenceId, queryString, res: 'success'});
			return res;
		});
}
...
/**
 * Removes users from a licence
 * @param {String} licenceId -
 * @param {Array} users -
 * @returns {Promise} response -
 */
function removeUsers (licenceId, users) {
	const operation = 'licenceDataClient.removeUsers';
	log.debug({operation, licenceId, users});

	const reqOptions = Object.assign({}, _getOptions(), {method: 'POST', body: JSON.stringify(users)});
	const url = `${apiUrl}/${licenceId}/remove`;
	return fetch(url, reqOptions)
		.then(res => {
			log.debug({operation, licenceId, users, res: 'success'});
			return res;
		});
}
