 ...
 * @param {ctxFilter} an object with one of the following properties set: access-licence-id, email-domain, ip-address
...
function getContexts(ctxFilter) {
	...
	.then(res => {
		...
		errors.parseStatus(res, `getContexts ${queryString}`);
		return res.json();
	})
	.then(acqObj => {
		return acqObj.items;
