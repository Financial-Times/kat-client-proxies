...
module.exports = (groupId, userId) => {
	...
	logger.debug({operation, groupId, userId});
	return myFTClient.getConceptsFollowedByGroup(groupId)
		...
		.then(groupConcepts => {
			logger.debug({operation, groupConcepts});
			if (Array.isArray(groupConcepts)) {
				return groupConcepts;
			}
			const msg = 'Group groupConcepts is not an array';
			logger.error({operation, groupConcepts, status: msg});
			throw new Error(msg);
		})
		.then(groupConcepts => {
			const conceptCount = groupConcepts.length;
			if (conceptCount > 0) {
				logger.silly({operation, subOp: 'groupConceptsFollowed', userId, groupId, groupConceptsCount: conceptCount});
				...
				return myFTClient.getConceptsFollowedByKatUser(userId)
					...
					.then(conceptsResp => {
						if (Array.isArray(conceptsResp)) {
							...
							logger.silly({operation, subOp: 'userConceptsFollowed', userId, groupId, userConceptsCount: userConceptIds.length});
							...
							if (newConceptsToFollow.length === 0) {
								logger.silly({operation, subOp: 'noNewConceptsToFollow', userId, groupId});
								return {user: {uuid: userId, group: groupId, status: 'synchronisationIgnored', reason: 'noNewConceptsToFollow'}};
							}

							logger.silly({operation, subOp: 'newConceptsToFollow', userId, group: groupId, newConceptsToFollow});
							...
							return myFTClient.addConceptsFollowedByKatUser(userId, newConceptsToFollow, followProps)
								.then(() => {
									...
									logger.silly({operation, subOp: 'setEmailDigestPreference', userId});
								...
								.then(() => {
									...
									logger.silly({operation, subOp: 'kinesis.write', userId});
						...
						logger.error({operation, conceptsResp, status: msg});
						throw new Error(msg);
					});

			}
			logger.silly({operation, subOp: 'noGroupConceptsToFollow', userId, groupId});
