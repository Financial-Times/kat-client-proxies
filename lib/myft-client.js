...
function getUsersWithEmailDigestPreference(uuid) {
	const url = `${config.myFTURL}/license/${uuid}/preference/email-digest/preferred/user`;
	const options = Object.assign({}, myFTOptions);
	...
	.then((res)=>{
		...
		return res.json()
		.then ((users)=>{
			log.debug({operation:'getUsersWithEmailDigestPreference', uuid, users: users.map((user)=>{return user.uuid;})});
			return users;
		});
	});
}
...
function getConceptsFollowedByGroup(uuid){
	getConceptsFollowed('group', uuid);
}
 ...
 */
function getUserFollowingConceptForLicence(licenceUuid, conceptUuid){
...
function getScopedRelationships(scopedNode, scopedUuid, node, uuid, relationship, relatedNode){
	const url = `${config.myFTURL}/${scopedNode}/${scopedUuid}/${node}/${uuid}/${relationships}/${relatedNode}`;
	const options = Object.assign({}, myFTOptions);
	return fetch(url, options)
	.then((res)=>{
