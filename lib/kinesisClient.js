...
function writeToKinesis(uuids, event, eventData) {
  ...
  while (records.length) {
    ...
    const chunk = records.splice(0, config.kinesisChunkSize);
    const chunkData = Object.assign({}, params, { Records: chunk });

    const logResponse = (res) => {
      logger.info({operation, chunkNr, uuids, event, res: JSON.stringify(res)});
      return res;
    };
    const retryOrDie = (error) => {
      if (retryCount < config.fetchOptions.maxRetries) {
        retryCount++;
        logger.warn({operation, chunkNr, msg: `Retry #${retryCount}`, uuids, event});
        return kinesis.putRecords(chunkData).promise().then(logResponse).catch(retryOrDie);
      }
      logger.error({operation, chunkNr, msg: `Operation FAILED after ${config.fetchOptions.maxRetries} retries`, uuids, event, error});
      return null;
    };
