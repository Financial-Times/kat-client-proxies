...
/**
 * Writes to Kinesis
 * @param {Array/String} uuids - takes an Array of uuids Or a single
 * @param {String} event -
...
function writeToKinesis (uuids, event, eventData) {
  ...
  logger.info({operation, uuids, event, dontWriteType: typeof config.DONT_WRITE_TO_KINESIS, dontWrite: config.DONT_WRITE_TO_KINESIS, uuidsIaA:  typeof uuids});
  ...
  if(!Array.isArray(uuids) && typeof uuids === 'string') {
    ...
    logger.info({operation, subOp: 'horible mutation so uuids.map works 🙀'});
    uuids = [uuids];
  ...
  if (!!config.DONT_WRITE_TO_KINESIS) {
    ...
    logger.info({operation, subOp: 'ignored', uuids, isuuidArray:Array.isArray(uuids), event});
