 ...
 */
//function getUsersForLicence(uuid){// TODO: WIP
 ...
 * @return {Promise} array of users
 */
//function getUsersForGroup(uuid){// TODO: WIP
...
function addUsersToGroup(groupUUID, userUUIDs, relationshipProperties, options){
	...
	return addRelationships('group', groupUUID, 'member', 'user', userUUIDs, relationshipProperties, options)
		.then(res=>{
			return parseJSONResponse(res, `${operation} group: ${groupUUID} users: ${userUUIDs}`)
				.then (users => {
...
function removeUsersFromGroup(licenceUUID, userUUIDs, options){
	...
	return removeRelationships('group', licenceUUID, 'member', 'user', userUUIDs, options)
		.then(res=>{
			return parseJSONResponse(res, `${operation} group: ${licenceUUID} users: ${userUUIDs}`)
				.then (users => {
					log.debug({operation, licenceUUID, users});
					return users;
				});
...
**/
//function addGroupsToLicence(licenceUUID, groupUUIDs, relationshipProperties, options){// TODO: WIP
