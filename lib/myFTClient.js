 ...
 * @private
 */
function _addRemoveKatUsers(method, node, nodeId, userIds, relProp, options, noResultParse) {
  const operation = `🐱 myFTClient.addRemoveUsers 🐱 - ${method} ${node}`;
  const uuidStr = JSON.stringify(userIds);
  const propStr = JSON.stringify(relProp);
  const optStr = JSON.stringify(options);
  ...
  let thePromise = _addRemoveKatRelationships(method, node, nodeId, myftConst.memberRelName, myftConst.userNodeName, userIds, relProp, options);
...
function _addRemoveConceptsFollowed(method, node, nodeId, newConceptsToFollow, followProps) {
  log.debug({operation: '_addRemoveConceptsFollowed'});
...
function _removeKatConceptsFollowedByNode(node, nodeUUID, conceptUUIDs, noResultParse) {
  ...
  let thePromise = _addRemoveKatRelationships('DELETE', node, nodeUUID, myftConst.followedRelName, myftConst.topicNodeName, conceptUUIDs);
 ...
 * @private
 */
function _multiAddRemoveKatRelationships(method, node, nodeIds, relIds, relProp, memberFollows, options) {
  const operation = 'myFTClient.multiAddRemoveRelationships';
  const relIdsStr = JSON.stringify(relIds);
  const propsStr = JSON.stringify(relProp);
  const optStr = JSON.stringify(options);
...
function _createAndTriggerKatRelationshipRequest(method, node, data, params, nodeId, memberFollows = false) {
  ...
  log.debug({operation: '🐱🐱 _createAndTriggerKatRelationshipRequest',node});
 ...
 */
 function _doRelationshipRequest(method, theUrl, data, params) {
  ...
  log.debug({operation: '🐱🐱🐱🐱🐱🐱🐱_doRelationshipRequest', method, theUrl, data, params});
...
function addConceptsFollowedByUser(userUUID, conceptUUIDs, relationshipProperties) {
  log.debug({operation: `addConceptsFollowedByUser`, msg: 'Depreciate in favor of addConceptsFollowedByKatUser'});
...
function addConceptsFollowedByKatUser(userUUIDs, conceptUUIDs, relationshipProperties) {
  log.debug({operation: `addConceptsFollowedByKatUser`});
 ...
 * @return {Promise} response -
**/
function addConceptsFollowedByKatGroup(groupUUID, conceptUUIDs, relationshipProperties) {
  return _addRemoveKatConceptsFollowed('POST', myftConst.groupNodeName , groupUUID, conceptUUIDs, relationshipProperties);
}
