 ...
 * @param {String} apiUrlOverride - Optional override to the default MyFT API url,
 *                                  used mainly for gradually rolling out/testing
 *                                  a new version of the API.
 * @returns {Promise} response -
 * @private
 */
function _getNodesFollowingConcept(licenceId, conceptId, nodeType, apiUrlOverride) {
  const operation = `myFTClient.getNodesFollowingConcept`;
  log.debug({operation, licenceId, conceptId, nodeType, apiUrlOverride});

  return _createAndTriggerScopedRequest('GET', myftConst.licenceNodeName, licenceId, myftConst.topicNodeName, conceptId, myftConst.followedRelName, nodeType, apiUrlOverride)
    .then(helpers.parseJsonRes)
    .then(res => {
 ...
 * @private
 */
function _createAndTriggerScopedRequest(method, node, nodeId, relatedNode, relatedNodeId, relationship, relatedType, data, params, apiUrlOverride) {
  const apiUrl = apiUrlOverride || config.MYFT_API_URL;
  const theUrl = `${apiUrl}/${node}/${nodeId}/${relatedNode}/${relatedNodeId}/${relationship}/${relatedType}`;
 ...
 * @returns {Promise} response -
 */
function getUsersFollowingConcept(licenceId, conceptId, apiUrlOverride) {
  return _getNodesFollowingConcept(licenceId, conceptId, myftConst.userNodeName, apiUrlOverride);
}
 ...
 * @returns {Promise} response -
 */
function getGroupsFollowingConcept(licenceId, conceptId, apiUrlOverride) {
  return _getNodesFollowingConcept(licenceId, conceptId, myftConst.groupNodeName, apiUrlOverride);
}
