...
function _addRemoveUsers(method, node, nodeId, userIds, relProp, options, noResultParse) {
  ...
  thePromise.then(res => {
    ...
    logObj.res = (noResultParse !== true) ? JSON.stringify(res): 'DONE';
...
function _removeConceptsFollowedByNode(node, nodeUUID, conceptUUIDs, noResultParse) {
  ...
  thePromise.then(res => {
    const logObj = {operation, nodeUUID, conceptUUIDs: conceptStr};
    logObj.res = (noResultParse !== true) ? JSON.stringify(res): 'DONE';
...
function setEmailDigestPreference(uuid, preference, options) {
  return _multiAddRemoveRelationships('POST', 'user', uuid, 'preferred', 'preference', [{uuid: 'email-digest'}], preference, options);
}
