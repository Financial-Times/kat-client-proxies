 ...
 * @private
 */
function _addRemoveKatConceptsFollowed(method, node, nodeId, newConceptsToFollow, followProps) {
  log.debug({operation: '🐱_addRemoveConceptsFollowed🐱', args: JSON.stringify(newConceptsToFollow)})
  return _multiAddRemoveKatRelationships(method, node, nodeId, 'follows', myftConst.topicNodeName, newConceptsToFollow, followProps);
}
 ...
 * @private
 */
function _multiAddRemoveKatRelationships(method, node, nodeId, rel, relType, relIds, relProp, options) {
  const operation = '🐱myFTClient.multiAddRemoveRelationships🐱';
  const relIdsStr = JSON.stringify(relIds);
  const propsStr = JSON.stringify(relProp);
  const optStr = JSON.stringify(options);
  ...
  return Promise.map(idChunks, (chunk, i) => {
    ...
    return _createAndTriggerKatRelationshipRequest(method, node, undefined, rel, relType, undefined, chunkData, params)
      .then(helpers.parseJsonRes)
      .then(res => {
        log.debug({operation, i, node, chunk, relIds: conceptIdsStr, relProp: propsStr, options: optStr, res: 'success'});
        return res;
      })
      .catch(err => {
...
function _createAndTriggerKatRelationshipRequest(method, node, data, params, nodeId, memberFollows = false) {
  ...
  if(memberFollows && nodeId && node === 'group') {
    //constructs url to handle relationship request for specific groupIds
     theUrl += `/${nodeId}/user`;
  } else if(!memberFollows && !nodeId === 'group') {
 ...
 * @return {Promise} response -
**/
function addConceptsFollowedByKatUser(userUUID, conceptUUIDs, relationshipProperties) {
  log.debug({opertation: `🐱addConceptsFollowedByKatUser🐱`, relProps: JSON.stringify(relationshipProperties)});
  return _addRemoveKatConceptsFollowed('POST', myftConst.userNodeName, userUUID, conceptUUIDs, relationshipProperties);
}
