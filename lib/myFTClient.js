...
function getConceptsFollowed(node, uuid) {
	...
	.then((res)=>{
		return parseJSONResponse(res, `${operation} ${uuid}`)
		.then((concepts)=>{
			log.debug({operation, uuid, count: concepts.count, concepts:JSON.stringify(concepts)});
			return concepts;
		});
	});
}
...
function addRelationships(node, nodeUUID, relationship, relatedType, relatedUUIDs, relationshipProperties, options){
	...
	const opts = Object.assign({}, {body:JSON.stringify(body)}, fetchOptions, {method:'POST'});
	log.silly({operation:'addRelationships', url, body:JSON.stringify(body)});
	log.debug({operation:'addRelationships', url});
	return fetch(url, opts)
	.then((res)=>{
