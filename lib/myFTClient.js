...
function getEmailDigestPreference(uuid) {
	...
	.then((res)=>{
		errors.parseStatus(res, `emailDigestPreferences for user ${uuid}`);
		return res.json().
		then ((edp)=>{
...
function getUsersWithEmailDigestPreference(uuid) {
	...
	.then((res)=>{
		errors.parseStatus(res, `usersWithEmailDigestPreferences for licence ${uuid}`);
		return res.json()
		.then ((users)=>{
...
function getConceptsFollowed(node, uuid) {
	...
	.then(res=>{
		errors.parseStatus(res, `conceptsFollowed by ${node} ${uuid}`);
		return res.json();
	});
}
