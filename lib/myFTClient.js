...
function _addRemoveUsers(method, node, nodeId, userIds, relProp, options, noResultParse) {
  ...
  let thePromise = _addRemoveRelationships(method, node, nodeId, 'member', 'user', userIds, relProp, options);
  ...
  thePromise.then(res => {
    const logObj = {operation, nodeId, userIds: uuidStr, relProp: propStr, options: optStr};
    // if we want the response to be parsed
    if (noResultParse !== true) {
      logObj.res = JSON.stringify(res);
    }
...
function _addRemoveGroups(method, licenseId, groupIds, relProp, options, noResultParse) {
  ...
  let thePromise = _addRemoveRelationships(method, 'license', licenseId, 'member', 'group', groupIds, relProp, options);
  ...
  thePromise.then(res => {
    const logObj = {operation, licenseId, groupIds: uuidStr, relProp: propStr, options: optStr};
    // if we want the response to be parsed
    if (noResultParse !== true) {
      logObj.res = JSON.stringify(res);
    }
...
function _addRemoveConceptsFollowed(method, node, nodeId, newConceptsToFollow, followProps) {
  return _multiAddRemoveRelationships(method, node, nodeId, 'followed', 'concept', newConceptsToFollow, followProps);
}
...
function _removeConceptsFollowedByNode(node, nodeUUID, conceptUUIDs, noResultParse) {
  ...
  log.debug({operation, nodeUUID, conceptUUIDs});

  let thePromise = _addRemoveRelationships('DELETE', node, nodeUUID, "followed", "concept", conceptUUIDs);
  ...
  thePromise.then(res => {
    const logObj = {operation, nodeUUID, conceptUUIDs};
    // if we want the response to be parsed
    if (noResultParse !== true) {
      logObj.res = JSON.stringify(res);
    }
...
function _multiAddRemoveRelationships(method, node, nodeId, rel, relType, relIds, relProp, options) {
  ...
  if (Array.isArray(nodeId)) {
    ...
    while (idsClone.length) {
      idChunks.push(idsClone.splice(0, config.myFtBatchUserCount));
    }
  } else {
  ...
  return Promise.map(idChunks, (chunk, i) => {
    ...
    return _createAndTriggerRelationshipRequest(method, node, undefined, rel, relType, undefined, chunkData, params)
      .then(helpers.parseJsonRes)
      .then(res => {
        log.debug({operation, i, node, chunk, rel, relType, relIds: relIdsStr, relProp: propsStr, options: optStr, res: JSON.stringify(res)});
        return res;
      })
      .catch(err => {
        log.error({operation, i, node, chunk, rel, relType, relIds: relIdsStr, relProp: propsStr, options: optStr, err});
        return err;
      });
  }, {concurrency: config.myFtBatchUserConcurrency})
    .then(results => {
...
function _createAndTriggerScopedRequest(method, node, nodeId, relatedNode, relatedNodeId, relationship, relatedType, data, params) {
  const theUrl = `${config.myFTURL}/${node}/${nodeId}/${relatedNode}/${relatedNodeId}/${relationship}/${relatedType}`;
...
function setEmailDigestPreference(uuid, preference, options) {
  return _multiAddRemoveRelationships('POST', 'user', uuid, 'preferred', 'preference', 'email-digest', preference, options);
}
...
function removeUsersFromLicence(licenceUUID, userUUIDs, options) {
  return _addRemoveUsers('DELETE', 'license', licenceUUID, userUUIDs, undefined, options, true);
}
...
function removeUsersFromGroup(groupUUID, userUUIDs, options) {
  return _addRemoveUsers('DELETE', 'group', groupUUID, userUUIDs, undefined, options, true);
}
...
function removeConceptsFollowedByGroup(groupUUID, conceptUUIDs) {
  return _removeConceptsFollowedByNode('group', groupUUID, conceptUUIDs, true);
}
...
function removeConceptsFollowedByUser(userUUID, conceptUUIDs) {
  return _removeConceptsFollowedByNode('user', userUUID, conceptUUIDs, true);
}
