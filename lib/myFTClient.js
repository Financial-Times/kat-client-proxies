...
function getEmailDigestPreference(uuid) {
	...
	.then((res)=>{
		statusErrors.parse(res, `emailDigestPreferences for user ${uuid}`);
		return res.json().
		then ((edp)=>{
...
function getUsersWithEmailDigestPreference(uuid) {
	...
	.then((res)=>{
		statusErrors.parse(res, `usersWithEmailDigestPreferences for licence ${uuid}`);
		return res.json()
		.then ((users)=>{
...
function getConceptsFollowed(node, uuid) {
	...
	.then(res=>{
		statusErrors.parse(res, `conceptsFollowed by ${node} ${uuid}`);
		return res.json();
	});
}
