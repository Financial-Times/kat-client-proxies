...
/**
 * Gets nodes following concepts/topics as an individual
 * @param {String} licenceId -
 * @param {String} conceptId -
 * @param {String} nodeType - user|group
 * @returns {Promise} response -
 * @private
 */
function _getNodesFollowingConceptAsIndividual(licenceId, conceptId, nodeType) {
  const operation = `myFTClient._getNodesFollowingConceptAsIndividual`;
  const params = { followType: 'individual' };
  log.debug({operation, licenceId, conceptId, nodeType});

  return _createAndTriggerScopedRequestKat('GET', myftConst.licenceNodeName, licenceId, myftConst.topicNodeName, conceptId, myftConst.followedRelName, nodeType, null, params)
    .then(helpers.parseJsonRes)
    .then(res => {
      log.debug({operation, licenceId, conceptId, nodeType, res: 'success'});
      return res;
...
/**
 * Creates the url and triggers the request ot get the scoped related nodes from the
...
function getUsersFollowingConceptAsIndividual(licenceId, conceptId) {
  return _getNodesFollowingConceptAsIndividual(licenceId, conceptId, myftConst.userNodeName);
}
